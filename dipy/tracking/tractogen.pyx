...
from dipy.tracking.tracker_parameters cimport (TrackerParameters,
                                               TrackerStatus,
                                               func_ptr)
from dipy.tracking.propspeed cimport glide_propagator
...
# ==============================================================================
# Glide-specific tractogram generation functions
# ==============================================================================

def generate_tractogram_glide(double[:,::1] seed_positions,
                              double[:,::1] seed_directions,
                              StoppingCriterion sc,
                              TrackerParameters params,
                              ...
                              float buffer_frac=1.0,
                              bint save_seeds=0):
    """Generate a tractogram using glide tracking from seed points.

    This is the glide-specific version that works with PeakDirectionGen
    instead of PmfGen.
    ...
    sc : StoppingCriterion
        Stopping criterion for the streamlines.
    params : TrackerParameters
        Parameters for the streamline generation (with glide propagator).
    peak_gen : PeakDirectionGen
        Peak direction data container.
    affine : ndarray
        Affine transformation for the streamlines.
    nbr_threads : int, optional
        Number of threads to use for streamline generation.
    ...
    while seed_start < _len:
        ...
        generate_tractogram_c_glide(seed_positions[seed_start:seed_end],
                                    seed_directions[seed_start:seed_end],
                                    nbr_threads, sc, params, peak_gen,
                                    streamlines_arr, length_arr, status_arr)
...
cdef void generate_tractogram_c_glide(double[:,::1] seed_positions,
                                      ...
                                      int* lengths,
                                      StreamlineStatus* status):
    """Generate tractogram using glide tracking (C implementation).

    This is the glide-specific C implementation with OpenMP parallelization.
    ...
    for i in prange(_len, nogil=True, num_threads=nbr_threads):
        stream = <double*> malloc((params.max_nbr_pts * 3 * 2 + 1) * sizeof(double))
        stream_idx = <int*> malloc(2 * sizeof(int))
        status[i] = generate_local_streamline_glide(&seed_positions[i][0],
                                                     &seed_directions[i][0],
                                                     stream,
                                                     stream_idx,
                                                     sc,
...
cdef StreamlineStatus generate_local_streamline_glide(double* seed,
    ...
    """Generate a single streamline using glide tracking.

    This is the glide-specific version for single streamline generation.
    ...
    status_forward = TRACKPOINT
    for i in range(1, params.max_nbr_pts):
        # Call glide_propagator directly (not via function pointer to avoid type mismatch)
        if glide_propagator(&point[0], &voxdir[0], params, stream_data, peak_gen, &rng) == TrackerStatus.FAIL:
            break
        # update position
        for j in range(3):
            point[j] += voxdir[j] * params.inv_voxel_size[j] * params.step_size
